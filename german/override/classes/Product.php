...
class Product extends ProductCore
{
	...
	public function __construct($id_product = null, $full = false, $id_lang = null, $id_shop = null, Context $context = null)
	{
		...
		*/
		...
		if ($full && $this->id)
		{
			$this->available_now   = !empty($this->available_now)   ? $this->available_now   : Configuration::get('GC_AVAILABLE_NOW',   (int)$id_lang);
			$this->available_later = !empty($this->available_later) ? $this->available_later : Configuration::get('GC_AVAILABLE_LATER', (int)$id_lang);
		}
	}
	...
	)
	{
		...
		*/
		...
		$sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, pl.*, MAX(product_attribute_shop.id_product_attribute) id_product_attribute, MAX(image_shop.`id_image`) id_image, il.`legend`, m.`name` AS manufacturer_name,
					DATEDIFF(
						p.`date_add`,
						DATE_SUB(
							NOW(),
	...
	public static function getProductProperties($id_lang, $row, Context $context = null)
	{
		...
		/*
		| GC German 1.5.6.0 | 20131022
		| Verbesserte Berechnung der Preise bei hohen Stückzahlen
		| Standard Lieferzeit aus Datenbank ermitteln pl.*
		*/
		...
		$cache_key = $row['id_product'].'-'.$row['id_product_attribute'].'-'.(int)$id_lang.'-'.(int)$usetax;
		...
		$row['id_image'] = Product::defineProductImage($row, (int)$id_lang);
		...
		$row['packItems'] = $row['pack'] ? Pack::getItemTable($row['id_product'], (int)$id_lang) : array();
		...
		$row['available_now']   = !empty($row['available_now'])   ? $row['available_now']   : Configuration::get('GC_AVAILABLE_NOW',   (int)$id_lang);
		$row['available_later'] = !empty($row['available_later']) ? $row['available_later'] : Configuration::get('GC_AVAILABLE_LATER', (int)$id_lang);
